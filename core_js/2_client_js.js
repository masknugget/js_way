...
{ /* Style apply to element with id="clock" */
    ...
    24
    pt
    sans; /* Use a big bold font */
    background: #ddf; /* On a light bluish-gray background */
    padding: 10
    px; /* Surround it with some space */
    border: solid
    black
    2
    px; /* And a solid black border */
    border - radius
:
    10
    px; /* Round the corners (where supported) */
}
<
/style>
< /head>
...
doc.-- >
< h1 > Digital
Clock < /h1> <!-- Display a title -->
< span
id = "clock" > < /span> <!-- The time gets inserted here -->
    < /body>
    < /html>

    < script
src = "../../scripts/util.js" > < /script>
...
onchange = "order.options.giftwrap = this.checked;" >
    ...
    < a
href = "javascript:new Date().toLocaleTimeString();" >
    What
time
is
it ?
...
< a
href = "javascript:alert(new Date().toLocaleTimeString());" >
    Check
the
time
without
...
< a
href = "javascript:void window.open('about:blank');" > Open
Window < /a>
...
do {
    ...
    try {
        r = "Result: " + eval(e);
    } /* Try to evaluate the expression */ catch (ex) {
        r = ex;
    } /* Or remember the error instead */
...
form >
< fieldset
id = "fields" > < /fieldset> <!-- Dialog body filled in by script below -->
    < div
style = "text-align:center" > < !--Buttons
to
dismiss
the
dialog-- >
< button
onclick = "okay()" > Okay < /button> <!-- Set return value and close -->
    < button
onclick = "cancel()" > Cancel < /button> <!-- Close with no return value -->
    < /div>
    < script >

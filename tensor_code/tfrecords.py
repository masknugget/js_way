...
# tfrecords的创建很简单，就是将每一组“样本数据”组装成一个Example对象，
# 这个对象是遵循protocol buffer协议的；然后将这个Example对象序列化成字符串；
# 最后用tf.python_io.TFRecordWriter写入相应的tfrecords文件即可。大致步骤如下：
...
x_test = x_test / 255.0     # (60000, 28, 28)
...
def decode_record(record):
    image = tf.io.decode_raw(
        record['image_raw'], out_type=dataType, little_endian=True,
